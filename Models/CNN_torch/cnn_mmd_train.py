...
# This section ensures that the script can find other modules in the project
...
class CNN_MMD_learner(object):
    ...
    def train_cnn(self, save_path):
        ...
        train_dataset = CNN_DataGenerator_torch(mode='train', ways=self.ways, shot=self.shots)
        test_dataset = CNN_DataGenerator_torch(mode='test', ways=self.ways, shot=self.shots)
        valid_dataset = CNN_DataGenerator_torch(mode='validation', ways=self.ways, shot=self.shots)
    ...
    def test_cnn(self, load_path):
        # === CHANGED: Optimized model loading for both CPU and GPU ===
